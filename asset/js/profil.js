    ...
    init() {
        ...
        this.slides.forEach(slide => {
            ...
            slide.addEventListener('click', (e) => {
                ...
                // Check if it's a video item FIRST (priority over images)
                if (video || playButton || videoThumbnail) {
                    // Prevent the click from propagating to avoid other event handlers
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    if (videoModal) {
                        ...
                        const videoId = playButton ? playButton.getAttribute('data-video-id') : null;
                        
                        if (videoId && videoPlayer && videoPlayer.tagName === 'IFRAME') {
                            // YouTube video - update iframe source
                            videoPlayer.src = `https://www.youtube.com/embed/${videoId}?rel=0&modestbranding=1&controls=1`;
                            
                            // Open video modal using the shared function
                            if (window.openVideoModal) {
                                ...
                                // Add autoplay for YouTube iframe
                                if (videoPlayer.src && !videoPlayer.src.includes('autoplay=1')) {
                                    const separator = videoPlayer.src.includes('?') ? '&' : '?';
                                    videoPlayer.src = videoPlayer.src + separator + 'autoplay=1';
                                }
                            }
                        } else if (video) {
                            // Local video - use Video.js
                            let vjsPlayer = videojs.getPlayer('videoPlayer');
                            if (!vjsPlayer) {
                    ...
                    return; // Early return to prevent image modal logic
                } else if (img) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    imageGrid.addEventListener('click', function(e) {
        ...
        if (gridItem) {
            ...
            if (itemData) {
                // Check if it's a video thumbnail FIRST (priority over images)
                const video = gridItem.querySelector('video');
                const playButton = gridItem.querySelector('.play-button');
                const videoThumbnail = gridItem.querySelector('.video-thumbnail');
                ...
                if (video || playButton || videoThumbnail) {
                    ...
                    // This is a video item - handle video modal
                    const videoId = playButton ? playButton.getAttribute('data-video-id') : null;
                    ...
                    if (videoId) {
                        ...
                        // Local video - use original logic
                        const videoPlayer = document.getElementById('videoPlayer');
                        if (videoPlayer) {
                    ...
                    // This is a regular image item - handle image modal
    ...
    // Video Modal logic for iframe/YouTube embed
    const videoModal = document.getElementById('videoModal');
    const videoClose = document.getElementById('videoClose');
    const videoPlayer = document.getElementById('videoPlayer');
    
    // Get ALL play buttons instead of just one with specific ID
    ...
    function openVideoModal() {
        if (videoModal) {
            ...
            // For YouTube iframe, we can add autoplay parameter
            if (videoPlayer && videoPlayer.tagName === 'IFRAME') {
    ...
    // Make openVideoModal globally accessible for the 3D carousel
    ...
    function closeVideoModal() {
        if (videoModal) {
            ...
            // Stop YouTube video by removing and re-adding iframe
            if (videoPlayer && videoPlayer.tagName === 'IFRAME') {
                const currentSrc = videoPlayer.src;
                videoPlayer.src = '';
                setTimeout(() => {
                    videoPlayer.src = currentSrc.replace('&autoplay=1', '').replace('?autoplay=1', '');
                }, 100);
            }
        }
    }
    ...
    allPlayButtons.forEach(playButton => {
        playButton.addEventListener('click', (e) => {
            ...
            // Check if this button has a specific video ID
